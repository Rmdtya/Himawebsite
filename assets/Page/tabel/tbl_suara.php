  ...
  <section class="home-section">
	...
	<div class="table-responsive">
		<div class="table-wrapper">
          ...
          <?php
                    $i = 1;
                    $query2 = mysqli_query($koneksi, "SELECT voting_calon, jumlah_voting, RANK() OVER (ORDER BY jumlah_voting DESC) AS peringkat FROM (SELECT voting_calon, COUNT(*) AS jumlah_voting FROM tbl_pemilu GROUP BY voting_calon) AS hasil_voting ORDER BY jumlah_voting DESC");
                    while($data = mysqli_fetch_array($query2)) { ?>
                      ...
                      <tr>
                        ...
                        <td>
                            <?php $calon = $data['voting_calon'];
                            echo $data['voting_calon']; ?>
                        </td>
                        <td>
                            <?php if($calon == 1){
                                echo "Ramaditya Firdaus";
                              } else if($calon == 2){
                                echo "Muhammad Rafid Miftah";
                              } else if($calon == 3 ){
                                echo "Cheey Adya Zahra";
                              }?>
                        </td>
                        <td>
                            <?php echo $data['jumlah_voting']; ?>
                        </td>
                        <td>
                            <?php echo $data['peringkat']; ?>
                        </td>
                      </tr>
        ...
        $(document).ready(function () {
          $('#pemilu').DataTable({
            "columns": [
                   {"data": "No"},
                   {"data": "Calon Nomor"},
                   {"data": "Nama Calon", orderable: false},
                   {"data": "Jumlah Suara"},
                   {"data": "Peringkat"}
                  ],
                  aoColumnDefs: [
